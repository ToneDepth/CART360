...
void live()
{
  ...
  //Adds the frequency of both the pressed button and the values corresponding to the photocell, and places them
  //at a set duration established by the variable duration.
  if (digitalRead(NOTE_IN_PIN) == HIGH) {
    tone(BUZZER_PIN, analogRead(NOTE_IN_PIN) + analogRead(PHOTO_PIN), duration);
    //Sets value of duration to 100
    duration == 100;
  }
}
...
void recordWithDuration()
{
  ...
  if (countNotes < MAX_NOTES) {
    ...
    //Added so a note isn't constantly being played. Waits for button to be pushed
    if (analogRead(NOTE_IN_PIN == HIGH)) {
    ...
    if (testNote > theAdjuster && (timePassed < MAX_PRESS_TIME)) {
      ...
      */
      if (activeNoteButton == false) {
        //Bypassedd using function here due to an error occuring that wouldn't allow the program to upload.
        startTime = millis();
...
**************************************************************************/
void startUpTimer() {
  //Sets start time to the current value of millis.
  startTime = millis();
  //Sets value to true so that button values can be recorded.
  activeNoteButton = true;
}
...
**************************************************************************/
void updateTimer() {
  //Takes the value of millis and subtracts it by startTime. This gives a value between when startTime got its value and when this function was called on.
...
void updateArraysWithNoteAndTimings() {
  //Condition seeing if current note doesn't bypass the array limit.
  if (countNotes < MAX_NOTES)
  {
    // add a note to the array if have a value
    activeFrequency = analogRead(NOTE_IN_PIN);
    if (testNote > 0) {
      //Sets current value of the array to the value of active frequency.
      notes[countNotes] = activeFrequency; // possible vals?
...
int getPhotoFrequency() {
  //Gets the initial frequency provided by the photocell.
...
void playWithDuration() {
  for (int i = 0; i < MAX_NOTES - 1; i++) {
    tone(BUZZER_PIN, notes[i] + runningAverageBuffer[i], duration);
    delay(duration);
    delay(200);
